...
describe('UserGateway Property-Based Testing (Fast-Check)', () => {
  let gateway: any;

  beforeEach(() => {
    // Mock dependencies
    const mockUserService = { send: jest.fn() };
    const mockConfigService = { get: jest.fn().mockReturnValue('secret') };

    gateway = new UserGateway(mockUserService as any, mockConfigService as any);
  });
      ...
      fc.assert(
        fc.property(fc.anything(), (handshake) => {
          ...
          try {
            ...
            // Should never throw regardless of how malformed the handshake is
